---
name: produktrecht-produktrecht-anpassen
...
> Opencode-Port von `produktrecht/skills/produktrecht-anpassen/SKILL.md`. Urspruenglicher Skill-Name: `produktrecht-anpassen`.
...
1. **Konfiguration lesen.** `.opencode/profile/produktrecht.md` lesen (und `./unternehmens-profil.md` eine Ebene darüber). Wenn die Plugin-Konfiguration nicht existiert oder noch `[PLATZHALTER]`-Werte enthält, sagen:
...
5. **Für gemeinsames-Profil-Änderungen** (Unternehmensname, Branche, Jurisdiktionen, Praxissetting, Phase): nach `./unternehmens-profil.md` schreiben und vermerken:
