---
name: datenschutzrecht-datenschutzrecht-kaltstart-interview
...
> Opencode-Port von `datenschutzrecht/skills/datenschutzrecht-kaltstart-interview/SKILL.md`. Urspruenglicher Skill-Name: `datenschutzrecht-kaltstart-interview`.
...
1. **Voraussetzungs-Check.**
   - `.opencode/profile/datenschutzrecht.md` prüfen.
   - Wenn befüllt (keine `[PLATZHALTER]`) und kein `--redo`: Bestätigung einholen, bevor überschrieben wird.
   - Wenn altes Cache-Profil unter `.opencode/plugins/cache/…` vorhanden: automatisch übertragen, dem Nutzer mitteilen.
