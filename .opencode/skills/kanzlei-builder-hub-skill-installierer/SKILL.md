---
name: kanzlei-builder-hub-skill-installierer
...
> Opencode-Port von `kanzlei-builder-hub/skills/skill-installierer/SKILL.md`. Urspruenglicher Skill-Name: `skill-installierer`.

# Skill-Installer

Folge dem nachstehenden Ablauf lückenlos. Kurzübersicht der Pflichtschritte:

1. **Zulassungsliste lesen.** `.opencode/plugins/config/kanzlei-builder-hub/positivliste.yaml`. Im restriktiven Modus und bei nicht gelisteter Quelle: Ablehnen. Im permissiven Modus: Warnung ausgeben und fortfahren.
...
Lese `.opencode/plugins/config/kanzlei-builder-hub/positivliste.yaml`.  
...
- Lese-/Schreibanweisungen außerhalb des Skill-eigenen Verzeichnisses — insbesondere auf `.opencode/`, CLAUDE.md, Shell-Konfigurationen
...
- **Dateischreibpfade** — schreibt eine Anweisung in `.opencode/`, CLAUDE.md, `.gitignore`, `ausloeser/` oder ähnliche umgebungsverändernde Pfade?
...
- Eigenständig: `.opencode/skills/[skill-name]/`
...
Eintrag in `.opencode/plugins/config/kanzlei-builder-hub/installations-protokoll.yaml` mit: Skill-Name, Quell-Registry, Herausgeber, Installationsdatum, Version, Zulassungslistenmodus, Lizenz, Lizenzquelle, Einsatzkontext sowie Aktualitätsfelder.
