---
name: gewerblicher-rechtsschutz-schutzrechts-verlaengerungs-monitor
description: "Geplanter Agent, der das IP-Portfolio-Register liest, fällige Fristen berechnet und einen nach Dringlichkeit geordneten Fristenbericht veröffentlicht. Läuft standardmäßig wöchentlich. Sendet Meldungen an den Kanal in `.opencode/profile/gewerblicher-rechtsschutz.md` → Verlängerungshinweise. Auslöser: \"was verlängert sich\", \"IP-Fristen\", \"Portfolio-Prüfung\", \"IP-Verlängerungsbericht\" oder nach Plan."
mode: subagent
permission:
  edit: ask
  bash: ask
...
> Opencode-Port von `gewerblicher-rechtsschutz/agents/schutzrechts-verlaengerungs-monitor.md`. Claude-spezifische Tool- und Kurzmodellangaben wurden entfernt; externe MCP-Server muessen in `opencode.json` bewusst aktiviert werden.
...
1. `.opencode/profile/gewerblicher-rechtsschutz.md` lesen, um das Benachrichtigungsziel (Slack-Kanal, E-Mail-Verteiler oder direkt) und die Arbeitsprodukt-Kopfzeilenregeln zu erhalten.
