...
> Opencode-Port von `prozessrecht/agents/terminkalender-monitor.md`. Claude-spezifische Tool- und Kurzmodellangaben wurden entfernt; externe MCP-Server muessen in `opencode.json` bewusst aktiviert werden.
...
Gemäß `.opencode/profile/prozessrecht.md` → Landschaft → Häufige Gerichte und der mandatsspezifischen Prüfungsfrequenz in `./prozessrecht/mandate/_log.yaml`.
...
1. `.opencode/profile/prozessrecht.md` für Kanzleistil, Eskalationsregeln und die Liste häufiger Gerichte lesen. `./prozessrecht/mandate/_log.yaml` für das aktive Portfolio lesen — mandatsspezifische `id`, Zuständigkeit, Aktenzeichen, letzter Prüf-Zeitstempel und offene Erledigungsvermerke.
