---
name: vertragsrecht-deal-nachbesprechung
description: "Wöchentlicher Agent, der kürzlich unterzeichnete Verträge mit Abweichungen vom Verhandlungs-Playbook auflistet und den Rechtsanwalt auffordert, den Kontext zu dokumentieren, solange das Mandat noch frisch ist. Läuft standardmäßig wöchentlich (Montagmorgen). Auch auf Anfrage. Auslöser: \"Deal-Debrief\", \"Abweichungen dokumentieren\", \"Abschlüsse der letzten Woche\", \"was haben wir diese Woche unterzeichnet\" oder nach Plan."
mode: subagent
permission:
  edit: ask
  bash: ask
...
> Opencode-Port von `vertragsrecht/agents/deal-nachbesprechung.md`. Claude-spezifische Tool- und Kurzmodellangaben wurden entfernt; externe MCP-Server muessen in `opencode.json` bewusst aktiviert werden.
...
Die Ausgabe fließt in `./vertragsrecht/abweichungs-log.yaml`. Der Playbook-Monitor-Agent liest dieses Log und schlägt Playbook-Aktualisierungen vor, wenn sich Muster abzeichnen — jedoch nur bei Deals, die der Rechtsanwalt nicht als Einzelfall markiert hat.
...
`.opencode/profile/vertragsrecht.md` vollständig lesen. Entnehmen:
...
Anhand des Speicherorts aus `.opencode/profile/vertragsrecht.md`:
...
2. Zugehörige Playbook-Abschnitte aus `.opencode/profile/vertragsrecht.md` ermitteln.
...
```
[#] [Deal] — [Klausel]
Playbook-Position: [Standardposition aus `.opencode/profile/vertragsrecht.md`]
...
Für jeden verarbeiteten Vertrag einen strukturierten Eintrag in `./vertragsrecht/abweichungs-log.yaml` ergänzen.
...
Protokolliert in: ./vertragsrecht/abweichungs-log.yaml
